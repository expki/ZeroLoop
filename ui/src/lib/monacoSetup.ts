// Self-hosted Monaco Editor setup with selective ESM imports.
// Uses explicit worker configuration and imports all basic languages.

// Configure workers before any editor features are imported
...
// All editor features (command palette, find/replace, folding, minimap, etc.)
import 'monaco-editor/esm/vs/editor/editor.all'

// Rich language services (validation, completions, diagnostics)
import 'monaco-editor/esm/vs/language/typescript/monaco.contribution'
import 'monaco-editor/esm/vs/language/css/monaco.contribution'
import 'monaco-editor/esm/vs/language/json/monaco.contribution'
import 'monaco-editor/esm/vs/language/html/monaco.contribution'

// All basic language syntax highlighting
...
// The API surface — must be imported after all contributions are registered
import * as monaco from 'monaco-editor/esm/vs/editor/editor.api'
import { loader } from '@monaco-editor/react'
import { registerGhostTextProvider } from '../extensions/monacoGhostText'
import { api } from '../services/api'

loader.config({ monaco })

// Register ghost text provider once globally (not per editor instance)
registerGhostTextProvider(monaco, async (prefix, suffix, filename, signal) => {
  const result = await api.smartCodeComplete(prefix, suffix, filename, signal)
  return result.text
})
